        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="table-full-wrapper angularTabs" >
                <ul class="nav nav-tabs">
                    ...
                    <li class="<?=(Auth::user()->hasRole('subreg'))?'active':''; ?>" ><a href="#reg-areas" data-toggle="tab">ΠΕΡΙΦΕΡΕΙΑΚΕΣ ΕΝΟΤΗΤΕΣ</a></li>
                    {{--@endif--}}

                    @if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('gov') ||  Auth::user()->hasRole('reg') ||  Auth::user()->hasRole('subreg') ||  Auth::user()->hasRole('mnc') )
                    <li class="<?=(Auth::user()->hasRole('mnc'))?'active':''; ?>"><a href="#municipalities" data-toggle="tab">ΔΗΜΟΙ</a></li>
                    @endif
                </ul>
                ...
                <div class="tab-content" >
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <tr ng-repeat = "region in regions | filter:query | orderBy:order_:orderDir | startFrom:currentPage*pageSize | limitTo:pageSize "> 
                                        <td><a href="{{url('region')}}/{!region.id!}">{!region.perifereia!}</a></td>
                    ...
                    {{--@if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('gov') ||  Auth::user()->hasRole('reg') ||  Auth::user()->hasRole('subreg') )--}}
                    <div class="tab-pane fade in <?=(Auth::user()->hasRole('subreg'))?'active':''; ?>" id="reg-areas" ng-controller="gorillaSubRegionsSumsCtr">
                        <div class="filters">
                            <div class="col-sm-6 col-xs-12">
                                <div class="perpage">
                                    <select ng-change="pageSizeChange()" ng-model="queryPages">
                    ...
                    @if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('gov') ||  Auth::user()->hasRole('reg') ||  Auth::user()->hasRole('subreg') ||  Auth::user()->hasRole('mnc') )
                    <div class="tab-pane fade in <?=(Auth::user()->hasRole('mnc'))?'active':''; ?>" id="municipalities" ng-controller="gorillaMunicipalitiesCtr">
                        <div class="filters">
                            <div class="col-sm-6 col-xs-12">
                                <div class="perpage">
                                    <select ng-change="pageSizeChange()" ng-model="queryPages">
