...
class RegionsModel extends Model {
    ...
    public function getPerifereies($year = 2014){
        ...
        foreach ($results as $key => $region){
            $empRow = new \stdClass();
            $empRow = $results[$key];
            $catsSums = RegionsModel::getCatsPerRegion($region->id,$year);
    ...
    public function getMainChartData($year,$district = null,$customerId = 0){
        ...
        foreach($categories as $key => $value){
            $jsonOutput[$value] = RegionsModel::getMainChart($year,$key,0,$district,$customerId);
        }

        $jsonOutput['totalQTY']         = RegionsModel::getTotalQty($year,$district);
        $jsonOutput['categoriesSUM']    = RegionsModel::getCategoriesSum($year,$district);
        $jsonOutput['totalProducers']   = RegionsModel::getTotalProducers($year,$district); 
        $jsonOutput['totalDromologia']   = RegionsModel::getTotaDromologia($year,$district); 
        $jsonOutput['totalMonades']     = RegionsModel::getTotaMonades($year,$district);
        //$jsonOutput['totalForthga']     = RegionsModel::getTotaTrucks($year,$district);
        $jsonOutput['totalForthga'] = '-';
        $jsonOutput['temperature']      = RegionsModel::getTemperature($year,$district);
        $jsonOutput['material']         = RegionsModel::getMaterial($year,$district);
        $jsonOutput['confiscation']     = RegionsModel::getConfiscation($year,$district);
        ...
        $jsonOutput['mex'] = RegionsModel::getMex($year);
        $jsonOutput['regions'] = RegionsModel::getPerifereies($year);
